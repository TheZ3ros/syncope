...
abstract class AbstractJPAAnySearchDAO extends AbstractAnySearchDAO {
    ...
    protected static final String SELECT_DISTINCT_ANY_ID_FROM = "SELECT DISTINCT any_id FROM ";
    ...
    protected AnySearchNode getQuery(
        ...
        if (not) {
            clause.append(anyId(svs)).append(NOT_IN_CLAUSE);
        } else {
            clause.append(anyId(svs)).append(IN_CLAUSE);
        }
    ...
    protected AnySearchNode getQuery(
        ...
        clause.append(SELECT_DISTINCT_ANY_ID_FROM).
                append(svs.relationship().name()).append(WHERE_CLAUSE).
    ...
    protected AnySearchNode getQuery(
        ...
        clause.append(SELECT_DISTINCT_ANY_ID_FROM).
                append(svs.membership().name()).append(WHERE_CLAUSE).
                append(subwhere).
                append(")) ");
    ...
    protected AnySearchNode getQuery(
        ...
        clause.append(SELECT_DISTINCT_ANY_ID_FROM).
                append(svs.role().name()).append(WHERE_CLAUSE).
    ...
    protected AnySearchNode getQuery(
        ...
        clause.append("SELECT DISTINCT group_id AS any_id FROM ").
                append(new SearchSupport(AnyTypeKind.USER).membership().name()).append(WHERE_CLAUSE).
        ...
        if (not) {
            clause.append("AND ").append(anyId(svs)).append(NOT_IN_CLAUSE);
        } else {
            clause.append("OR ").append(anyId(svs)).append(IN_CLAUSE);
        }

        clause.append("SELECT DISTINCT group_id AS any_id FROM ").
                append(new SearchSupport(AnyTypeKind.ANY_OBJECT).membership().name()).append(WHERE_CLAUSE).
                append(members.stream().
                        map(key -> "any_id=?" + setParameter(parameters, key)).
                        collect(Collectors.joining(" OR "))).
                append(')');
    ...
    protected AnySearchNode getQuery(
        ...
        return switch (checked.cond().getType()) {
            case ISNULL ->
                ...
                defaultSV(svs),
                checked.cond().getSchema() + (not ? IS_NOT_NULL : " IS NULL"));

            case ISNOTNULL ->
                new AnySearchNode.Leaf(
                defaultSV(svs),
                checked.cond().getSchema() + (not ? " IS NULL" : IS_NOT_NULL));
    ...
    protected AnySearchNode.Leaf buildAdminRealmsFilter(
        ...
        if (realmKeys.isEmpty()) {
            return new AnySearchNode.Leaf(defaultSV(svs), StringUtils.substringAfter(anyId(svs), '.') + IS_NOT_NULL);
        }
        ...
        node.asLeaf().ifPresentOrElse(
                leaf -> {
                    ...
                    if (counters.computeIfAbsent(leaf.getFrom(), view -> false) && !leaf.getClause().contains(" IN ")) {
                        where.add(anyId(svs) + IN_CLAUSE
                                + "SELECT any_id FROM " + leaf.getFrom().name()
                                + WHERE_CLAUSE + leaf.getClause().replace(leaf.getFrom().alias() + ".", "")
                                + ")");
                    } else {
    ...
    protected String buildCountQuery(
        ...
        String queryString = new StringBuilder("SELECT COUNT(DISTINCT ").append(anyId(svs)).append(") ").
                ...
                append(WHERE_CLAUSE).append(buildWhere(where, root)).
                toString();
    ...
    protected String buildSearchQuery(
        ...
        queryString.append(WHERE_CLAUSE).append(buildWhere(where, root));
